...
var TicketInfo = Backbone.View.extend({
    initialize: function() {
        var self = this;
        this.loadTicketsFromDatabase(function(items) {
            var data = { "items": items };
            var ticketInfoBody = _.template($("#table-ticket-info").html());
            $("#ticket-sold-info").html(ticketInfoBody(data));
        });
    ...
    loadTicketsFromDatabase: function(callback) {
        $.ajax({
            url: "/api/tickets",
            method: "GET",
            success: function(response) {
                ...
                _.each(response, function(row) {
                    var name = row.name;
                    var number = row.number_of_seats;
                    var bookedSeats = convertIntToSeatNumbers(row.seats);
                    items.push({ names: name, numbers: number, seats: bookedSeats });
                });
                callback(items);
            },
    ...
    events: {
        "click #clear-history-btn": "clearBookingHistory"
    },
